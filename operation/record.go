...
func (value Record) validate() error {
	for _, field := range []string{value.ID, value.Owner, value.Kind, value.ActionKey, value.ResourceType, value.IdempotencyKey, value.RequestFingerprint, value.RequestedBy, value.Reason, value.Status, value.CreatedAt, value.UpdatedAt} {
